  ...
  return (
    <div className="flex items-center gap-3 rounded-xl bg-muted/40 px-4 py-3 border border-border">
      <Avatar className="size-10 shrink-0">
        <AvatarImage src={provider.image ?? getAvatarPlaceholder(provider.name ?? "")} />
...
export default function BookServicePage() {
  ...
  const selectedDate = form1.watch("scheduledDate");
  const selectedCategoryId = form1.watch("categoryId");
                      ...
                      <div className="grid gap-2 sm:grid-cols-2">
                        {provider.services!.map((svc) => (
                          <button
                            ...
                            className={cn(
                              "flex items-start gap-3 rounded-xl border p-3 text-left transition-all focus-visible:ring-2 focus-visible:ring-ring",
                              field.value === svc.id
                          ...
                          >
                            ...
                            {field.value === svc.id && (
                              <CheckCircle2 className="size-4 text-brand-orange ml-auto shrink-0 mt-0.5" />
                            )}
                          </button>
                        ))}
                      </div>
        ...
        {step === 1 && (
          <form onSubmit={handleNext2} className="space-y-5">
            ...
            <div className="flex gap-3">
              ...
              <Button
                type="submit"
                className="h-10 w-full bg-brand-orange hover:bg-(--brand-orange)/90 text-white gap-2"
              >
                Continue
                <ArrowRight className="size-4" />
              </Button>
          ...
          <div className="space-y-5">
            <Card>
              ...
              <CardContent className="space-y-4 text-sm">
                ...
                {s3Data?.notes && (
                  <>
                    <ConfirmRow icon="📝" label="Notes" value={s3Data.notes} />
                    <Separator />
                  </>
                )}
