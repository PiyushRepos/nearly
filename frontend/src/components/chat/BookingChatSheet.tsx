    ...
    useEffect(() => {
        ...
        const onNewMessage = (msg: Message) => {
            mutate((prev) => {
                ...
                const tempIdx = prev.findIndex((m) => String(m.id).startsWith("temp-") && m.content === msg.content);
                if (tempIdx !== -1) {
                    const next = [...prev];
                    next[tempIdx] = msg;
                    return next;
        ...
        <Sheet open={open} onOpenChange={setOpen}>
            ...
            <SheetContent className="w-full sm:max-w-md p-0 flex flex-col h-full bg-card">
                <SheetHeader className="p-4 border-b shrink-0 flex flex-row items-center justify-between border-border bg-muted/20">
                    <div>
                        <SheetTitle className="text-lg flex items-center gap-2" style={{ fontFamily: "Fraunces, serif" }}>
                ...
                <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-muted/10 flex flex-col" ref={scrollRef}>
                    {!messages ? (
                        ...
                        messages.map((msg, i) => {
                            ...
                            const showTime =
                                ...
                                format(new Date(msg.createdAt), "h:mm a") !== format(new Date(messages[i + 1].createdAt), "h:mm a");

                            return (
                                <div key={msg.id} className={cn("flex flex-col gap-1 w-full max-w-[85%]", isMe && "self-end items-end")}>
                                    <div
                                        className={cn(
